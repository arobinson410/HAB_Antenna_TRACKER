...
IMEI = "300234064904600"     #######   YOU MUST ENTER YOUR IMIE NUMBER HERE   ##########
...
def moveTiltServo(position):
        ...
        if servoAttached:
            ...
            if(position < 70):          #80 degrees upper limit
                    ...
                    moveTilt = "1:" + str(-(1.4509433962 * position) + 179.056603774)
            print "Preparing to Tilt"            
            s.write(moveTilt)
...
def movePanServo(position):
        ...
        if servoAttached:
            s = serial.Serial(str(servoCOM), baudrate = servoBaud, timeout = servoTimeout)
            print "Preparing to Pan"
            movePan = "0:" + str(((1.41732283465 * (255-position))-180))
